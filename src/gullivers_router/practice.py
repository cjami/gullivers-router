...
GradeQuality = Literal["correct", "partially_correct", "incorrect"]
...
_SYSTEM_PROMPT = (
    ...
    "and alternate correct code. Use score 1.0 for correct, 0.5 for partially correct, and 0.0 "
    "for incorrect. Respond only with JSON."
)
...
def score_practice_with_context(options: PracticeOptions, context: PracticeContext) -> PracticeReport:
    ...
    judge = context.chat_factory(context.settings.judge)
    records = _grade_tasks(tasks, references, candidates, judge, workers=options.workers)
...
def _grade_tasks(
    ...
    *,
    ...
    with futures.ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        lock = Lock()
        submitted_futures = {
            pool.submit(_grade_one, task, references[task.task_id], candidates[task.task_id], judge): task.task_id
...
def _grade_one(
    ...
    grade = call_with_retry(lambda: complete_structured(judge, messages, PracticeGrade))
    return {
        "task_id": task.task_id,
...
def _report(records: Sequence[GradeRecord]) -> PracticeReport:
    ...
    correct = sum(1 for record in records if record["quality"] == "correct")
    partial = sum(1 for record in records if record["quality"] == "partially_correct")
    incorrect = sum(1 for record in records if record["quality"] == "incorrect")
...
def _log_summary(report: PracticeReport, output_path: Path) -> None:
    ...
    _log(
        ...
        f"({summary['correct']} correct, {summary['partially_correct']} partial, {summary['incorrect']} incorrect) "
        f"-> {output_path}"
    )
